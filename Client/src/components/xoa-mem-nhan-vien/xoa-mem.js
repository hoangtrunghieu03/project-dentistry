...
import { getnhanvienxoamem, xoanguoidung, khoiphucnguoidung } from '../../actions/AdminthuAction';
...
function Nhanvien(props) {
    ...
    useEffect(() => {
        dispatch(getnhanvienxoamem());
    }, [dispatch ]);

    const nhanviens = useSelector((state) => state.nhanviens.nhanvien);
    ...
    console.log(nhanviens)
      ...
      const handkhoiphuc = async (nhanvien_id) => {
        const shouldUpdate = window.confirm('Bạn có muốn khôi phục nhân viên không?');
        if (shouldUpdate) {
