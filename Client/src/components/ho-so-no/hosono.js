...
import { getAllhosono } from '../../actions/ThanhtoanAction';
import { deletememhosobenhan } from '../../actions/AdminthuAction';
import moment from 'moment';


function Hosonos(props) {
  ...
  useEffect(() => {
    dispatch(getAllhosono('hoan-tat'));
  }, [dispatch]);

  console.log(userInfo.status);
  

  const medicalrecords = useSelector((state) => state.hoadons.hosono);
  ...
  const handdeletemem = async (medicalrecord_id) => {
    const shouldUpdate = window.confirm('Bạn có muốn xóa hồ sơ nợ không?');
    if (shouldUpdate) {
  ...
  return (
    <div>
    <div className="more4">
      <h2 className="h2ds">Danh sách hồ sơ nợ</h2>
    </div>
            ...
            medicalrecords.map((medicalrecord) => (
             ...
             <td> 
              {userInfo.status === 'admin' ? (
                <Link to={`/ho-so-chi-tiet-quan-ly/${medicalrecord._id}`}>
                  <i className="ti-pencil-alt"></i>
                </Link>
              ) : (
                <Link to={`/ho-so-no-chi-tiet/${medicalrecord._id}`}>
                  <i className="ti-pencil-alt"></i>
                </Link>
              )}
              </td>
    ...
    //         <br />
    //         <br />
    //         <Link to={`/ho-so-no-chi-tiet/${medicalrecord._id}`} className="">
    //             Thực hiện
